...
class Akatustef_Akatustef_Model_Pagar extends Akatusbase_Akatusbase_Model_Pagar
{
    public function __construct()
    {
        parent::__construct();
        $this->_code = 'akatustef';
        $this->_formBlockType = 'akatustef/form_pay';
        $this->_infoBlockType = 'akatustef/info_pay';
    }
    ...
    public function assignData($data) {
        ...
        $info->setCheckTefbandeira($data->getCheckTefbandeira());
        $info->setCheckFormapagamento("tef");
    ...
    public function validate() {
    	...
    	if($formapagamento === "tef") {
            if(empty($tefbandeira)){
                $errorCode = 'invalid_data';
                $errorMsg = $this->_getHelper()->__('Escolha o banco pelo qual deseja realizar o débito online.');
	...
	public function gerarXML($order) {
		...
		<carrinho>			
			...
			$info = $this->getInfoInstance();  
			$formapagamento = $this->getCheckFormapagamento();
			$tefbandeira = $info->getCheckTefbandeira();
	...
	public function enviaGateway($order, $xml) {
		...
		} else if ($resposta == "Aguardando Pagamento") {
			
			$url_base = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB);	
			$url_destino = $this->getTefUrl();
