...
class Akatuscartao_Akatuscartao_Model_Pagar extends Mage_Payment_Model_Method_Abstract
{
    ...
    */
	...
	public function GeraXML(Varien_Object $payment, $amount){
		...
		<carrinho>			
			...
			foreach ($items as $itemId => $item)
			{
				//$preco= number_format($item->getPrice(),'2','','');
                        ...
                        //deme55Ry
                        //$fullprice = $this->helper('tax')->getPrice($item, $item->getPrice(), true);

                        //$preco_item = number_format($item->getPrice(), 2, '', '');
                        $preco_item = number_format($item->getPrice(), 2, '', '');
                        Mage::Log('produto>>preco: '.$preco_item);
                        //Mage::Log('preco com taxas:'.$fullprice);
		...
		$xml.='</carrinho>';
                //discount_amount
                Mage::Log("XML pronto para o envio...");
		...
		else
		{
			...
			if($data["resposta"]["status"]["value"]=="Em Análise"){
				...
				//$url_destino='https://www.akatus.com/boleto/';
	              ...
	              //$info = $this->getInfoInstance();
                  //$formapagamento=$info->getCheckFormapagamentocartao();
